...
# Main Issue :
# ------------------------------------------------
# data_interval_start rely on DAG excution date or logical date window
# solve the SQL dead lock issue when deleting staging table, isolating by using run_id from the Airflow context.
# solve the mongodb(NoSQL) nested structure issue. 
...
def extract_mongo_to_staging(**context):
    ...
    # loaded directly into a staging table and only a reference (path / batch_id) would be passed via XCom
...
def run_soda_quality_check(**context):
    ...
    # This line is equivalent to running this in the terminal inside your Airflow container
    ...
    # Airflow does NOT do: It does not 1.log what your Python function is doing/ 2.subprocess stdout & stderr automatically/ 3.not interpret success/failure context for you.
...
with DAG(
    ...
    dagrun_timeout=timedelta(hours=2),  # cap total runtime per dag run
